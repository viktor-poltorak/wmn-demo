...
class Manager_CompaniesController extends Eve_Controller_AdminAction
{
    ...
    {
        $this->_assign('companies', $this->_companies->getAll());
    ...
    public function addAction()
    {
        ...
        $this->_display('companies/edit.tpl');
    }
    ...
    public function saveAction()
    {
        ...
        if ($title == '') {
            $errors[] = $this->errors->companies->no_title;
        }
        if ($description == '') {
            $errors[] = $this->errors->companies->no_description;
        }

        if ($category_id == 0) {
            $errors[] = $this->errors->companies->no_category;
        }
        if ($producer_id == 0) {
            $errors[] = $this->errors->companies->no_category;
        }
        ...
        if ($errors) {
            ...
            if ($this->_request->id) {
                ...
                if (!$id)
                    $this->_redirect('/manager/companies');
                $this->_companies->update($bind, $id);
            } else {
                $bind['date_posted'] = new Zend_Db_Expr('NOW()');
                $this->_companies->insert($bind);
            }
            $this->_redirect('/manager/companies');
        }
    }
    ...
    public function deleteAction()
    {
        $id = (int) $this->_request->id;
        if ($id) {
            $product = $this->_companies->load($id);
    ...
    protected function _uploadSmallImage()
    {
        ...
        if (isset($this->_uploads['image']['name'])) {
            ...
            if ($image) {
                ...
                if ($this->_request->id) {
                    $product = $this->_companies->load($this->_request->id);
